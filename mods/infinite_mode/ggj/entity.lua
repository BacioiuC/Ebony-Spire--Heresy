...
function entity:summonSpecialSpawner(_x, _y )
	local creatureName = " "
	if #self:makeSpawnListForLevel( ) > 0 then
		if  Game.grid[_x][_y] == 0 and self:isEntityAt(_x, _y) == false then
			creatureName = self:new(_x, _y, math.random(1, #self:makeSpawnListForTowerLevel( ) ), false)
		end
	end
		...
		if v.isBox == false then
			...
			if evaResult == nil then
				if v.state ~= "Follow" then
					...
					self:handleIdle(v)--self:handleFollowPlayer(v)
				end
...
function entity:handleIdle(_v)
	local v = _v
	local px, py = player:returnPosition( )
	--local bool = self:_isInSight(v.x, v.y, px, py)--self:checkForPlayer(_v)
	if self:_isInSight(v.x, v.y, px, py) == true then
		v.state = "Aware"
		self:handleAware(v)
	...
	if v ~= nil then
		...
		for x = -v.sightRange, v.sightRange do
			for y = -v.sightRange, v.sightRange do
				...
				if self:_isInSight(v.x, v.y, px, py) == false then
					seePlayer = false
				end
				if bool == true then -- it means there's a entity there (or player)
					if id == nil then -- clearly a player, since ID returns value for entity
						...
						local temp = {
							id = #evaluationTable + 1,
							value = self:getPerceivedDangerForEntity(id), -- getPerceivedDangerForEntity -- NO MORE ENTITY INFIGHTING
							x = v.x-x,
							y = v.y-y,
...
function entity:moveAtRandom(_v)
	...
	if v.destX == v.x and v.destY == v.y then
		...
		for x = -v.sightRange, v.sightRange do
			for y = -v.sightRange, v.sightRange do
				if v.x - x > 0 and v.x - x < 30 and v.y - y > 0 and v.y - y < 30 then
					if Game.grid[v.x-x][v.y-y] == 0 and self:isEntityAt(v.x-x, v.y-y) == false and v.x-x ~= px and v.y-y ~= py then
						eX = v.x-x
						eY = v.y-y
					end
				end
...
function entity:_calcDamage(_v,_type)
	...
	if _type == 1 then 
		...
		damage = damage + weaponDamage + math.random(1, 4)
	...
	if v.inventory ~= nil then
		...
		if bool == true then --we haz items
			...
			elseif _item._type == "Accessories" then
				...
				elseif rnd == 4 then
					log:newMessage(""..v.name.."Spins the ".._item.name.." between it's fingers!")					
				elseif rnd == 3 then
		...
		if nrEntities == 0 then
			...
			if spawnChance < 5 then
				for i = 1, rnd do
					...
					entity:summonSpecialSpawner( x, y )
				end
			end
...
function entity:_isInSight(x1,y1,x2,y2)
    local math = math
    ...
    if (roundnum ~= 1) and rngMap:isWallAt(x,y) and self:isEntityAt(x, y) then onemoreround = 1 end --was return false
